...
use interoptopus::{
    callback, constant, ffi_constant, ffi_function, ffi_service, ffi_service_ctor,
    ffi_service_method, ffi_surrogates, ffi_type, function, pattern, Inventory, InventoryBuilder,
};
use std::ffi::CStr;
...
impl Vec2 {
    #[ffi_service_ctor]
    pub fn new(x: f32, y: f32) -> Result<Self, MyFFIError> {
        Ok(Self { x, y })
    }

    pub fn error(&self) -> Result<(), MyFFIError> {
        Err(MyFFIError::OtherError)
    }
...
pub unsafe extern "C" fn my_function4(callback: InputString) {
    ...
    callback.call(AsciiPointer::from_cstr(
        CStr::from_bytes_with_nul(h.as_bytes()).unwrap(),
    ));
}
